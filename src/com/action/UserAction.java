...
public class UserAction extends HttpServlet {
	...
	private void doUpdate(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			String userPwd = request.getParameter("userPwd");
			//封装到对象中
			UserInfo ui = new UserInfo(userId,userName,userPwd);
	...
	private void toUpdate(HttpServletRequest request, HttpServletResponse response) {
		try {
			int userId = Integer.parseInt(request.getParameter("userId"));
			//1 通过userId来查询出该对象
			UserInfo ui = ubiz.findUserById(userId);
	...
	private void delete(HttpServletRequest request, HttpServletResponse response) {
		try {
			int userId = Integer.parseInt(request.getParameter("userId"));
			int i = ubiz.deteleUserById(userId);
			if(i > 0){
				this.findAllUsers(request,response);
			}
		} catch (Exception e) {
	...
	private void findAllUsers(HttpServletRequest request, HttpServletResponse response) {
		try {
			//1 使用BIZ的方法查询出所有用户记录
			List<UserInfo> ulist = ubiz.findAllUsers();
